...
//! and emits .clj + a C shim, following rules verified by hand against
//! runtime/src/diplomat/thingy.clj — the reference shape this generator's
//! output is meant to match. Scope is intentionally limited to the shapes
//! `runtime/src/diplomat/thingy.clj` and the crates in `examples/` exercise
//! — an unsupported shape panics loudly (or, for method-level shapes, is
//! skipped with an eprintln!) rather than silently emitting something wrong.
//!
//! Critical rule: ANY struct-by-value crossing, param or return, ALWAYS
//! goes through the shim — never emit a direct defcfn against a
//! struct-by-value symbol, even where SysV ABI register-packing would
//! happen to make it work (verified this occurs for sum_with's
...
fn main() {
    ...
    let template = "\
;; GENERATED by jolt-diplomat-backend — copy to your project's deps.edn and fill in paths.
;; Replace <CRATE_NAME> with the actual library name (e.g. \"mylib\").
{:jolt/native
 [{:name \"<CRATE_NAME>\"
   :darwin  [\"lib<CRATE_NAME>.dylib\"]
   :linux   [\"lib<CRATE_NAME>.so\"]
   :windows [\"<CRATE_NAME>.dll\"]}
  {:name \"<CRATE_NAME>_shim\"
   :darwin  [\"lib<CRATE_NAME>_shim.dylib\"]
   :linux   [\"lib<CRATE_NAME>_shim.so\"]
   :windows [\"<CRATE_NAME>_shim.dll\"]}]}
";
    let template_path = out_dir.join("deps.edn.template");
...
fn gen_opaque_clj(tcx: &hir::TypeContext, op: &hir::OpaqueDef, shim_c: &mut String) -> String {
    ...
    let _ = writeln!(out, "  Do not hand-edit — see backend/src/main.rs for the generator, README.md for how this fits together.\"");
...
fn is_variadic(m: &hir::Method) -> bool {
    if m.docs.is_empty() {
        return false;
    }
    let text = m.docs.to_markdown(
    ...
    text.lines().any(|l| l.trim() == "jolt-diplomat: variadic")
}

fn variadic_flag_for(m: &hir::Method) -> &'static str {
    if is_variadic(m) { " :&" } else { "" }
}
...
fn gen_method(
    ...
    if !needs_shim {
        ...
        let variadic_flag = variadic_flag_for(m);
        let _ = writeln!(out, "(ffi/defcfn ^:private c-{fn_name} \"{c_sym}\" [{}] {ret_ty}{blocking_flag}{variadic_flag})",
            arg_types.join(" "));
...
mod tests {
    ...
    #[test]
    fn variadic_marker_emits_ampersand_flag_on_defcfn() {
        let tcx = lower(r#"
            #[diplomat::bridge]
            mod ffi {
                #[diplomat::opaque]
                ...
                impl Logger {
                    /// jolt-diplomat: variadic
                    pub fn log(&self, level: u32) -> u32 {
                        level
                    }

                    pub fn plain(&self, level: u32) -> u32 {
                        level
                    }
                }
            }
        ...
        let m_log = find_opaque_method(&tcx, "Logger", "log");
        assert!(is_variadic(m_log), "the jolt-diplomat: variadic marker must be detected");
        assert!(gen_method(&tcx, "Logger", m_log, &mut out, &mut shim_c, &mut extra_requires));
        assert!(
            out.contains(":&"),
            "log's generated defcfn must carry :&, got:\n{out}"
        );

        let before_plain = out.len();
        let m_plain = find_opaque_method(&tcx, "Logger", "plain");
        assert!(!is_variadic(m_plain), "plain must not be detected as variadic");
        assert!(gen_method(&tcx, "Logger", m_plain, &mut out, &mut shim_c, &mut extra_requires));
        let plain_section = &out[before_plain..];
        assert!(
            !plain_section.contains(":&"),
