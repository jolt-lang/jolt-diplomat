...
cc "$GENERATED/generated_shim.c" \
  -I "$HEADERS" \
  -L "$DYLIB_DIR" -lregex_capi \
  -shared -fPIC \
  -o "$DEMO/libregex_shim.dylib" \
  -Wl,-rpath,"$DYLIB_DIR"

echo ""
echo "Done. Artifacts:"
echo "  $CAPI/target/release/libregex_capi.dylib"
echo "  $DEMO/libregex_shim.dylib"
echo "  $GENERATED/diplomat/regex.clj"
